...
- **Assertions:** Always web-first. Playwright retries them until the timeout.
...
- If a test fails or is skipped, log the exact reason in `notes` (e.g. "Frontend bug — 20 char validation missing").
...
- **Use official Playwright APIs:** `page.setInputFiles()` for uploads, `expect.poll()` for async polling, `page.route()` for network mocking. No `page.evaluate()` to forcibly click hidden elements — fix the underlying DOM/wait issue.
