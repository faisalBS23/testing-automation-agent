...
> Universal instructions for AI coding agents (Claude Code, puku-cli, Cursor, Codex, GitHub Copilot, Gemini Code Assist, Aider, Continue, Cody, Windsurf, JetBrains AI, Sourcegraph, etc.).
...
>
> Block installed by `testing-automation-agent` (https://github.com/faisalBS23/testing-automation-agent). Edits inside the marker block will be replaced on update — edit outside freely.
...
- All new work goes to the active suite (default: the newest / V5) unless the human explicitly says "legacy/V3".
...
LAYER ∈ `SMOKE | E2E | API | REGRESSION`.
Reuse existing module/submodule codes from the matrix — **do not invent new ones**.
...
- **Waiting and synchronization (§3):**
  - **Preferred (Best):** `await page.waitForResponse(resp => resp.url().includes('/api/') && resp.status() === 200)`.
  - **Preferred (Good):** `await expect(page.getByTestId('spinner')).toBeHidden()`.
  - **Preferred:** `await page.waitForLoadState('networkidle')` after major navigations.
  - **Last resort only:** `page.waitForTimeout()` — add a `// reason:` comment explaining why no Playwright-native wait works (e.g. third-party animation, external iframe, PDF render).
- **Assertions (§4):** Always web-first. Playwright retries them until the timeout.
...
### 4. Update traceability matrix (§9)
...
- One matrix file per module. Don't create per-feature matrix files unless the team agrees.
- If a test fails or is skipped, log the exact reason in `notes` (e.g. "Script failed due to Frontend bug — 20 char validation missing").
...
- **Forbidden:** removing assertions, weakening expectations, commenting out checks to force green (§8).
...
- **Use official Playwright APIs** (§12): `page.setInputFiles()` for uploads, `expect.poll()` for async polling, `page.route()` for network mocking. No `page.evaluate()` to forcibly click hidden elements — fix the underlying DOM/wait issue.
...
## Communication rules (§13)
- **Never `git push` or commit to remote without explicit user permission.**
- **Ask before assuming.** If unsure about a requirement, locator, or flow — STOP and ask the user. Do not assume risky parts yourself without clarification.
...
- "add a smoke / e2e / regression / api test"
